...
#----------------------------------------------------------------------------
# Makefile used for the project: Weather-Station		W.Waetzig
#----------------------------------------------------------------------------
...
#----------------------------------------------------------------------------
# Specific OUTDIR option: output files are not poluting the source code
# directory, option added by Elektor LABs 2009
#----------------------------------------------------------------------------
...
OBJDIR = build
# Output files directory should be the same as OBJDIR then all generated files
# are not poluting the source code directory
OUTDIR = $(OBJDIR)
...
MSG_ARCHIVING = Archiving project:
...
GENDEPFLAGS = -MMD -MP -MF $(OUTDIR)/.dep/$(@F).d
...
ELFSIZE = $(SIZE) --target=$(FORMAT) $(OUTDIR)/$(TARGET).hex
#ELFSIZE = $(SIZE) --format=avr $(TARGET).elf
#ELFSIZE = $(SIZE) --format=sysv $(TARGET).elf
...
# Target: deepclean project.
deepclean: begin clean_list_extended end
...
clean_list :
	...
	$(REMOVE) $(SRC:%.c=$(OUTDIR)/%.d)
	$(REMOVE) $(SRC:%.c=$(OUTDIR)/%.i)
	$(REMOVEDIR) $(OUTDIR)/.dep
	$(REMOVEDIR) $(OUTDIR)

clean_list_extended : clean_list
	$(REMOVE) *~
	$(REMOVE) $(SRC:.c=.c~)

# Target: archive project.
archive: begin archive_list end

ARCHIVE_NAME = $(TARGET).rar
archive_list :
	@echo
	@echo $(MSG_ARCHIVING)
	$(ARCHIVE) $(ARCHIVE_NAME) makefile
	$(ARCHIVE) $(ARCHIVE_NAME) $(SRC:%.c=%.c)
	$(ARCHIVE) $(ARCHIVE_NAME) $(SRC:%.c=%.h)
	$(ARCHIVE) $(ARCHIVE_NAME) $(OUTDIR)/$(TARGET).hex
	$(ARCHIVE) $(ARCHIVE_NAME) $(OUTDIR)/$(TARGET).eep
